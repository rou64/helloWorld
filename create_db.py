...
with app.app_context():
    ...
    users = [
        {'username': 'student', 'email': 'student@umd.edu', 'first_name':'Imma', 'last_name': 'Student',
            'password': generate_password_hash(password='studentpw', method='sha256'), 'role': 'STUDENT'},
        {'username': 'manager', 'email': 'manager@umd.edu', 'first_name':'Joe', 'last_name': 'King',
            'password': generate_password_hash(password='managerpw', method='sha256'), 'role': 'MANAGER'},
        {'username': 'admin', 'email': 'admin@umd.edu', 'first_name':'Crystal', 'last_name': 'Ball',
            'password': generate_password_hash(password='adminpw', method='sha256'), 'role': 'ADMIN'},
        {'username': 'rou', 'email': 'rou@umd.edu', 'first_name': 'Raymond', 'last_name': 'Ou',
         'password': generate_password_hash(password='randompassword', method='sha256'), 'role': 'STUDENT'}
    ]
